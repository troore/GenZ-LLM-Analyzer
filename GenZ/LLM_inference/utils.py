...
def get_offload_system(system, total_memory_req, debug):
    ...
    if memory_offloaded > 0: 
        # new_offchip_BW = (total_memory_req ) / max(min(total_memory_req, total_device_memory)/unit.raw_to_unit(system.offchip_mem_bw, type='BW'), 
        new_offchip_BW = (total_memory_req ) / max(min(total_memory_req, total_device_memory)/system.get_offchip_mem_bw(), 
                                                   memory_offloaded/system.get_external_mem_bw()) 
...
def get_inference_system(system_name='A100_40GB_GPU', bits='bf16', ceff=1, meff=1):
    ...
    if isinstance(system_name, dict):
        if system_name.get('real_values',False) == True:
            ...
            OFFLOAD_MEM_BW = system_name.get('Slow_Memory_BW', 128)
    ...
    return System(unit, 
                  frequency=1000, 
                  flops=NUM_FLOPS,
                  off_chip_mem_size=(per_chip_memory*1024), 
                  compute_efficiency=ceff, 
                  memory_efficiency=meff,
                  offchip_mem_bw=OFFCHIP_MEM_BW, 
                  bits=bits, 
                  external_mem_bw=OFFLOAD_MEM_BW, 
                  interchip_mem_bw=C2C_BW, 
                  interchip_link_latency=C2C_LL)